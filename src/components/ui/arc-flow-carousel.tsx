...
export default function ArcFlowCarousel({
  ...
  // Source of truth for the list of cards. Falls back to placeholder data
  // when no items are passed so the demo keeps working out of the box.
  const effectiveItems: SmoothSliderItem[] =
    items && items.length > 0
      ? items
      : DEFAULT_META.map((m, i) => ({
          src: "",
          ...m,
        }));

  // How many card slots the wheel needs so the wrap point stays off screen.
  const [slotCount, setSlotCount] = useState(() => Math.max(effectiveItems.length, 12));
  ...
  const slots = Array.from({ length: slotCount }, (_, i) => effectiveItems[i % total]);
        ...
        {slots.map((item, i) => (
          ...
          >
            ...
            >
              ...
              {item.label ? (
                <span className="absolute bottom-3 left-1/2 -translate-x-1/2 px-3 py-1 rounded-full bg-dark/70 backdrop-blur-sm text-[0.78rem] tracking-[0.16em] text-white uppercase font-semibold whitespace-nowrap shadow-md">
                  {item.label}
                </span>
              ) : null}
            </div>
